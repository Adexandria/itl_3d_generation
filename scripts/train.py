...
class train:
        ...
        def run(self,current_epoch=1, json_file=None):
            ...
            for epoch in range(current_epoch, self.num_epochs + current_epoch):
                ...
                for gloss_embedding,keypoint,mask in self.train_loader: 
                    if gloss_embedding is None or keypoint is None or mask is None:
                        ...
                        continue  
                    gloss_embedding = gloss_embedding.unsqueeze(1).repeat(1, keypoint.size(1), 1)   # Ensure the input is of shape (1, 1, 768)
                    gloss_embedding = gloss_embedding.to(self.device)
                    keypoint = keypoint.to(self.device)  
                    mask = mask.to(self.device)             
                    # print(f"Gloss embedding shape: {gloss_embedding.shape}, Keypoint shape: {keypoint.shape}, Mask shape: {mask.shape}")
                    # Forward pass
